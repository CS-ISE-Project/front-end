...
function handleLogin(){
  ...
  return (
    <div className='flex flex-col text-sm md:text-md w-[80vw] md:w-[30vw]  gap-8'>
        <h2 className='text-2xl md:text-4xl'>S'inscrire</h2>
        ...
        <form className='flex flex-col gap-3' onSubmit={handleSubmit}>
          ...
          {isSubmitting && errors.lastName && <div className='text-Rose100'>{errors.lastName}</div>}
          {isSubmitting && errors.firstName && <div className='text-Rose100'>{errors.firstName}</div>}
            <label >Email</label>
            <input 
            ...
            <button className='rounded-md text-white font-bold bg-[#8D92C9] p-3 md:p-4' >Se connecter</button>
        </form>
        <button className='flex justify-center items-center gap-4 rounded-md bg-[#F2F3F6] p-3 md:p-4'><img src="devicon_google.svg" alt="" />Connexion avec Google</button>
        <p className='self-center'>Avez vous deja un compte? <span className='cursor-pointer text-Blue66' onClick={handleLogin}>Se connecter</span></p>
    </div>
  )
}
